...
def main():
    ...
    for sample, label in samplesandlabels:
        ...
        samplewatch.Stop()
        sampletime = round(samplewatch.RealTime(), 2)
        print("\tSample skimming ended in {} minutes".format(str(sampletime/60)))

    skimwatch.Stop()
    skimtime = round(skimwatch.RealTime(), 2)
    print("HTT skimming ended in {} minutes".format(str(skimtime/60)))
