...
# Point XrootD to current directory for configuration
# os.environ["XRD_PLUGINCONFDIR"] = os.getcwd()
...
def skimandhistos(basepath):
    ...
    for sample, label in samplesandlabels:
        ...
        sample_file = basepath + sample + "_1.root"

        # Create RDataFrame
        # Uncomment to use only one file per sample
        df = RDataFrame("Events", sample_file, npartitions=1000)
...
def main():
    ...
    eosbasepath = ("root://eospublic.cern.ch/"
                   "/eos/root-eos/benchmark/CMSOpenDataHiggsTauTau/")

    kitstoragebasepath = "/storage/9/wunsch/benchmark/CMSOpenDataHiggsTauTau/"

    cephbasepath = "/ceph/wunsch/benchmark/CMSOpenDataHiggsTauTau/"
    ...
    t = ROOT.TStopwatch()
    skimandhistos(eosbasepath)
    t.Stop()
    realtime = round(t.RealTime(), 2)

    start_time = datetime.now()
    filename = f"htt_{start_time.year}_{start_time.month}_{start_time.day}_{start_time.hour}_{start_time.minute}.csv"
    outcsv = args.out if args.out else filename
    with open(outcsv, "a+") as f:
        f.write(str(realtime))
        f.write("\n")
    print(f"HTT Analysis ended in {realtime/60} minutes")
