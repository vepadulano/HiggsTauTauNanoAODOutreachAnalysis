...
const std::string samplesBasePath =
    "root://eospublic.cern.ch//eos/opendata/cms/upload/stefan/"
...
const std::vector<std::string> sampleNames = {
    "GluGluToHToTauTau", "VBF_HToTauTau", "DYJetsToLL",  "TTbar",
    "W1JetsToLNu",       "W2JetsToLNu",   "W3JetsToLNu", "Run2012B_TauPlusX",
    "Run2012C_TauPlusX",
};
 ...
 */
template <typename T> auto AddEventWeight(T &df, const std::string &sample) {
 ...
 * reconstructed as taus, e.g., electrons or jets that could fake such a
 * particle.
 */
template <typename T>
...
int main() {
  ...
  outfile.open("higgstautau_cpp_multithreaded.csv");
  ...
  for (int i = 1; i < 101; i++) {
    ...
    for (const auto &sample : sampleNames) {
      ...
      df9.Snapshot("Events", sample + "Skim.root", finalVariables);
    }
    time.Stop();
